...
void ADS1292_Init(SPI_HandleTypeDef *hspi)
{
    ...
    if(device_id == 0x73)
		{
			DBG_Printf("This device is ADS1292R\r\n");
			device_id =0;
			HAL_Delay(100);
		}
    else if(device_id ==0x53)
		{   
			DBG_Printf("This device is ADS1292\r\n");
      device_id =1;
		}
    else
   {
       DBG_Printf("the device is not recognized,please check the device\r\n");
       while(1)
       {
            DBG_Printf("ERROR ID:%02x\r\n", device_id);
       }
    }
